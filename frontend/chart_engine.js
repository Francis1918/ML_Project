// ============================================================
//  chart_engine.js -- Orquesta render + eventos + crosshair.
// ============================================================
...
class ChartEngine {
  constructor(market, els) {
    this.market = market;
    this.els    = els;   // { priceCanvas, atrCanvas, statusHover, statusWindow }
    ...
    this._mouse = null;          // { x, y, panel } en coords del canvas, o null
    this._hoverIndex = null;     // indice de vela bajo el cursor
  ...
  // Movimiento sobre un canvas concreto -> actualiza crosshair
  ...
  // Dibuja el crosshair en AMBOS paneles (vertical sincronizada).
  _draw_crosshair_all() {
    ...
    // Linea vertical (en ambos paneles), centrada en la vela
    ...
    // Linea horizontal solo en el panel del cursor
  ...
  // Actualiza la barra de estado: datos bajo el cursor + info de ventana.
  _update_statusbar() {
    const win = this.compute_window();

    // Info de ventana (siempre)
    if (this.els.statusWindow) {
      this.els.statusWindow.textContent =
        `TF ${this.market.timeframe} | barras ${win.count} | offset ${this.offset}`;
    }

    // Info bajo el cursor (si aplica)
    if (this.els.statusHover) {
      if (this._hoverIndex !== null &&
        ...
        this.els.statusHover.textContent =
          `${c.time}  O:${c.open} H:${c.high} L:${c.low} C:${c.close}  ATR:${atrTxt}`;
      } else {
